...
if printf '%s' "$cmd" | grep -qE '(^|[;&|([:space:]])codex[[:space:]]+(exec|e)([[:space:]]|$)'; then
  cat <<'JSON'
{"hookSpecificOutput":{"hookEventName":"PreToolUse","permissionDecision":"deny","permissionDecisionReason":"Raw 'codex exec' is blocked. Use the deterministic wrapper: ~/.claude/skills/codex-delegate/scripts/codex-run.sh <commit|implement|explore|ingest|review|hardest> [--escalate] [--effort E] [--write] [--web] [-C dir] \"<full-context prompt>\" then Read the OUT= file (see the codex-delegate skill). To bypass intentionally, prefix the command with CODEX_RAW=1 and state why."}}
JSON
fi
exit 0
